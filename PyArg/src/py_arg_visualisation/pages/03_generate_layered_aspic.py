...
left_column = dbc.Col([
    ...
    dbc.Row([dbc.Col(html.B('Number of literals')),
             dbc.Col([dbc.Input(type='number', min=0, max=500, step=2, value=6,
                                id='03-generate-layered-aspic-nr-literals-input'),
                      dbc.Tooltip('The desired number of literals in the argumentation system. '
                                  'Note that this number should be even.',
                                  target='03-generate-layered-aspic-nr-literals-input')])]),
    dbc.Row([dbc.Col(html.B('Number of  rules')),
             dbc.Col([dbc.Input(type='number', min=0, max=500, step=1, value=3,
                                id='03-generate-layered-aspic-nr-rules-input'),
                      dbc.Tooltip('The desired number of rules (strict or defeasible).',
                                  target='03-generate-layered-aspic-nr-rules-input')])]),
    dbc.Row([dbc.Col(html.B('Rule antecedent distribution')),
             dbc.Col([dbc.Input(id='03-generate-layered-aspic-rule-antecedent-distribution-input',
                                value='2: 1, 1: 2'),
                      dbc.Tooltip('Number of rules with a specific number of antecedents. '
                                  'These should be put in the format '
                                  '[nr_ants_1]: [nr_rules_1], [nr_ants_2]: [nr_rules_2], ...',
                                  target='03-generate-layered-aspic-rule-antecedent-distribution-input')])]),
    dbc.Row([dbc.Col(html.B('Literal layer distribution')),
             dbc.Col([dbc.Input(id='03-generate-layered-aspic-literal-layer-distribution-input',
                                value='0: 3, 1: 2, 2: 1'),
                      dbc.Tooltip('Number of literals on a specific "layer" (which is something '
                                  'like the height in the support graph). '
                                  'These should be put in the format '
                                  '[layer_nr_1]: [nr_literals_1], [layer_nr_2]: [nr_literals_2], ...',
                                  target='03-generate-layered-aspic-literal-layer-distribution-input'
                                  )])]),
    dbc.Row([dbc.Col(html.B('Strict rule ratio')),
             dbc.Col([dbc.Input(type='number', min=0, max=1, value=0,
                                id='03-generate-layered-aspic-strict-rule-ratio-input'),
                      dbc.Tooltip('Ration of strict rules over all rules, e.g. if this number is 0.8, '
                                  'then around 80% of the generated rules are expected to be strict.',
                                  target='03-generate-layered-aspic-strict-rule-ratio-input')])]),
    html.Br(),
...
layout = html.Div([
    ...
    html.P('This generator assumes that negation is used for contradiction. '
           'In addition, it does not generate any preference ordering between defeasible rules.'),
...
@callback(Output('03-generate-layered-aspic-output', 'children'),
          ...
          State('03-generate-layered-aspic-rule-antecedent-distribution-input', 'value'),
          State('03-generate-layered-aspic-literal-layer-distribution-input', 'value'),
...
def generate_layered_aspic_argumentation_system(
    ...
    argumentation_system_json = json.dumps(ArgumentationSystemToJSONWriter().to_dict(argumentation_system))
        ...
        [html.H2('Output'),
         dbc.Row([
            ...
            dbc.Col([html.B('Rules'), dbc.Textarea(
                value='\n'.join((str(rule) for rule in argumentation_system.rules)),
                style={'height': '300px'})])
         ]),
